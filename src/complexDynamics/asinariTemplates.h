...
template<typename T, class Descriptor> struct asinariTemplatesImpl;
...
static T bgk_ma2_nonsymmetric_equilibrium(plint iPop, Array<T,Descriptor<T>::d> const& j) {
    return asinariTemplatesImpl<T,typename Descriptor<T>::BaseDescriptor>
        ::bgk_ma2_nonsymmetric_equilibrium(iPop, j);
}

static T bgk_collision_stage1(Cell<T,Descriptor>& cell, T rhoBar, T invRho,
                              Array<T,Descriptor<T>::d> const& j, T jSqr, T prefactor)
{
    return asinariTemplatesImpl<T,typename Descriptor<T>::BaseDescriptor>
        ::bgk_collision_stage1(cell.getRawPopulations(), rhoBar, invRho, j, jSqr, prefactor);
}

static void bgk_collision_stage3(Cell<T,Descriptor>& cell, Array<T,Descriptor<T>::d> const& j, T prefactor) {
    asinariTemplatesImpl<T,typename Descriptor<T>::BaseDescriptor>
        ::bgk_collision_stage3(cell.getRawPopulations(), j, prefactor);
}
...
static T bgk_collision_stage1(Array<T,Descriptor::q>& f, T rhoBar, T invRho, Array<T,Descriptor::d> const& j,
                              T jSqr, T prefactor)
{
    for (plint iPop=0; iPop<Descriptor::q; ++iPop) {
        f[iPop] = dynamicsTemplatesImpl<T,Descriptor>::bgk_ma2_equilibrium(iPop, rhoBar, invRho, j, jSqr) -
                  prefactor * bgk_ma2_nonsymmetric_equilibrium(iPop, j);
    }
    return jSqr * util::sqr(invRho);
}

static void bgk_collision_stage3(Array<T,Descriptor::q>& f, Array<T,Descriptor::d> const& j, T prefactor) {
    for (plint iPop=0; iPop<Descriptor::q; ++iPop) {
...
template<typename T> struct asinariTemplatesImpl<T, descriptors::D2Q9DescriptorBase<T> > {
...
static T bgk_collision_stage1(Array<T,Descriptor::q>& f, T rhoBar, T invRho,
                              Array<T,Descriptor::d> const& j, T jSqr, T prefactor)
{
    T p = (T)1-prefactor;
