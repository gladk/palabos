...
class InterpolateCoarseToFineDynamics3D : public BoxProcessingFunctional3D_LL<T,Descriptor1,T,Descriptor2>
{
public:
    InterpolateCoarseToFineDynamics3D (
            RescaleEngine<T,Descriptor1>* rescaleEngine_,
            InterpolationEngine2D<T,Descriptor1>* interpolationEngine_, plint direction_, const Dot3D delta_[2] );
    virtual ~InterpolateCoarseToFineDynamics3D();
    InterpolateCoarseToFineDynamics3D(InterpolateCoarseToFineDynamics3D<T,Descriptor1,Descriptor2> const& rhs);
...
///
template<typename T, template<typename U> class Descriptor1, template<typename U> class Descriptor2>
class InterpolateCoarseToFineBoundaryDynamics3D : public BoxProcessingFunctional3D_LL<T,Descriptor1,T,Descriptor2>
{
public:
...
template<typename T, template<typename U> class Descriptor1, template<typename U> class Descriptor2>
class InterpolateCoarseToFineCornerDynamics3D : public BoxProcessingFunctional3D_LL<T,Descriptor1,T,Descriptor2>
{
public:
...
public:
    CoarseToFineConverter3D( BlockLattice3D<T,Descriptor> const& coarseLattice,
    ...
    FineGridBoundaryDynamics<T,Descriptor>& fineDynamics (
            Dot3D coarsePos, Dot3D delta )
    {
        return dynamic_cast<FineGridBoundaryDynamics<T,Descriptor>&> (
                   fineLattice.get( fineX(coarsePos.x) + delta.x,
                                    fineY(coarsePos.y) + delta.y,
                                    fineZ(coarsePos.z) + delta.z ).getDynamics() );
    }
