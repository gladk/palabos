...
#ifndef MULTI_GRID_2D_CPP
#define MULTI_GRID_2D_CPP
...
/// Constructors
MultiGridPeriodicitySwitch2D::MultiGridPeriodicitySwitch2D(MultiGrid2D *block_)
    : block(block_)
{ }

MultiGridPeriodicitySwitch2D::MultiGridPeriodicitySwitch2D(MultiGridPeriodicitySwitch2D const& rhs)
    :block(rhs.block),periodicityArray(rhs.periodicityArray)
{ }

MultiGridPeriodicitySwitch2D& MultiGridPeriodicitySwitch2D::operator=(MultiGridPeriodicitySwitch2D const& rhs)
{
    block = rhs.block;
...
MultiGridStatSubscriber2D::MultiGridStatSubscriber2D(MultiGridStatSubscriber2D const& rhs)
              : multiGrid(rhs.multiGrid), dimensionsX(rhs.dimensionsX),
                dimensionsT(rhs.dimensionsT)
...
/// Copy
MultiGridStatSubscriber2D& MultiGridStatSubscriber2D::operator=(MultiGridStatSubscriber2D const& rhs)
{
    multiGrid = rhs.multiGrid; // assign the pointer to the multigrid
    dimensionsX.assign(rhs.dimensionsX.begin(),rhs.dimensionsX.end()); // copy all spatial dimensions
    dimensionsT.assign(rhs.dimensionsT.begin(),rhs.dimensionsT.end()); // copy all temporal dimensions 
    return *this;
}
...
MultiGrid2D::MultiGrid2D( MultiGridManagement2D management_, 
...
MultiGrid2D::MultiGrid2D(MultiGrid2D const& rhs, Box2D subDomain, bool crop)
    : Block2D(rhs),
      management (
          extractManagement(rhs.getMultiGridManagement(), subDomain, crop ) ),
      behaviorLevel(rhs.behaviorLevel),
      periodicitySwitch(rhs.periodicitySwitch), maxProcessorLevel(rhs.maxProcessorLevel),
      statisticsOn(rhs.statisticsOn),
      statsSubscriber(this),
      scaleManager(rhs.scaleManager->clone())
{}
...
void loadBinaryGrid(MultiGrid2D& block, std::string fName, bool enforceUint){
    ...
    for (plint iLevel=0; iLevel<block.getNumLevels(); ++iLevel){
        base64StreamToUnSerializer (
                istr, block.getComponent(iLevel).getBlockUnSerializer (
                            block.getComponent(iLevel).getBoundingBox(),
                            global::IOpolicy().getIndexOrderingForStreams()),
                enforceUint );
    }
