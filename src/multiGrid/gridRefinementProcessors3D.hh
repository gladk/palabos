         ...
         std::vector<Dot3D>& delta )
{
    ...
    delta.push_back(Dot3D(delta[0].x+delta[1].x,delta[0].y+delta[1].y,delta[0].z+delta[1].z));
    ...
    Dot3D fineExcess   ( converter.fineX(x0) + delta[1].x,
                         converter.fineY(y0) + delta[1].y, 
    ...
    Dot3D startFineExcess ( converter.fineX(x0) - delta[0].x,
                            converter.fineY(y0) - delta[0].y, 
                            converter.fineZ(z0) - delta[0].z );
    bool startOutOfRange = ( !contained(startExcess, coarseLattice.getBoundingBox()) ||
                             !contained(startFineExcess, fineLattice.getBoundingBox()) );
    Dot3D endExcess ( x1 + delta[0].x, y1 + delta[0].y, z1 + delta[0].z );
    Dot3D endFineExcess ( converter.fineX(x1) + delta[0].x,
                          converter.fineY(y1) + delta[0].y, 
                          converter.fineZ(z1) + delta[0].z );
    bool endOutOfRange = ( !contained(endExcess, coarseLattice.getBoundingBox()) ||
                           !contained(endFineExcess, fineLattice.getBoundingBox()) );
    
    if (!startOutOfRange){
        interpolationEngine->pushToCouple (
        Array<Cell<T,Descriptor>,2>(coarseLattice.get(x0-delta[0].x,
                                                      y0-delta[0].y,
                                                      z0-delta[0].z),
    ...
    if (outOfRange && !startOutOfRange){
        ...
        if (!startOutOfRange && !outOfRange){
            interpolationEngine->pushToCouple (
                Array<Cell<T,Descriptor>,2>(coarseLattice.get( x0-delta[0].x+delta[1].x,
                                                               y0-delta[0].y+delta[1].y,
                                                               z0-delta[0].z+delta[1].z ),
                                            coarseLattice.get(  x0+delta[1].x,
                                                                y0+delta[1].y,
                                                                z0+delta[1].z)),
            *rescaleEngine );
    ...
    if (!startOutOfRange){
        ...
        interpolationEngine->interpolateFromCouple (
            converter.fineDynamics( x0,y0,z0,
                                    -delta[0].x, -delta[0].y,-delta[0].z).getDecomposedValues(whichTime), 0,1 );
    }
        ...
        if (!startOutOfRange){
            interpolationEngine->interpolateFromCouple (
                converter.fineDynamics( x0,y0,z0,
                                        -delta[0].x+delta[1].x,
                                        -delta[0].y+delta[1].y,
                                        -delta[0].z+delta[1].z ).getDecomposedValues(whichTime), 0,2 );
            interpolationEngine->interpolateFromAll(
                converter.fineDynamics( x0,y0,z0,
                                        delta[1].x,delta[1].y,delta[1].z ).getDecomposedValues(whichTime) );
        }
        else {
            interpolationEngine->interpolateFromCouple (
                converter.fineDynamics( x0,y0,z0,
                                        delta[1].x,
                                        delta[1].y,
                                        delta[1].z ).getDecomposedValues(whichTime), 0,1 );
        }
    }
    ...
    for (plint iX=x0; iX<=x1-delta[0].x; ++iX) {
        for (plint iY=y0; iY<=y1-delta[0].y; ++iY){
            for (plint iZ=z0; iZ<=z1-delta[0].z; ++iZ){
                interpolationEngine->pushToCouple (
                Array<Cell<T,Descriptor>,2>(coarseLattice.get(iX, iY, iZ),
                                             coarseLattice.get(iX+delta[0].x,
                                                               iY+delta[0].y,
                                                               iZ+delta[0].z)),
                *rescaleEngine );
                ...
                if (outOfRange){
                    ...
                    interpolationEngine->pushToCouple (
                        Array<Cell<T,Descriptor>,2>(coarseLattice.get( iX+delta[1].x,
                                                                       iY+delta[1].y,
                                                                       iZ+delta[1].z),
                                                     coarseLattice.get( iX+delta[2].x,
                                                                        iY+delta[2].y,
                                                                        iZ+delta[2].z)),
                    *rescaleEngine );
                }
                ...
                interpolationEngine->copyFrom (
                    converter.fineDynamics(iX,iY,iZ,
                                           2*delta[0].x,2*delta[0].y,2*delta[0].z).getDecomposedValues(whichTime), 1 );
                interpolationEngine->interpolateFromCouple (
                        converter.fineDynamics(iX,iY,iZ,
                                               delta[0].x, delta[0].y,delta[0].z).getDecomposedValues(whichTime), 0,1 );
                if (!outOfRange){
                    interpolationEngine->interpolateFromCouple (
                        converter.fineDynamics( iX,iY,iZ, 
                                            delta[1].x, delta[1].y, delta[1].z ).getDecomposedValues(whichTime), 0,2 );
                    interpolationEngine->interpolateFromAll(
                        converter.fineDynamics( iX,iY,iZ, 
                                               delta[2].x,delta[2].y,delta[2].z ).getDecomposedValues(whichTime) );
                }
            }
        }
    }
    ...
    if (!endOutOfRange){
        interpolationEngine->pushToCouple (
        Array<Cell<T,Descriptor>,2>(coarseLattice.get(x1,y1,z1),
                                    coarseLattice.get(x1+delta[0].x,
                                                      y1+delta[0].y,
                                                      z1+delta[0].z)),
        *rescaleEngine );
    }
    ...
    if (outOfRange && !endOutOfRange){
        ...
        if (!endOutOfRange && !outOfRange){
            interpolationEngine->pushToCouple (
                Array<Cell<T,Descriptor>,2>(coarseLattice.get(  x1+delta[1].x,
                                                                y1+delta[1].y,
                                                                z1+delta[1].z),
                                            coarseLattice.get( x1+delta[0].x+delta[1].x,
                                                               y1+delta[0].y+delta[1].y,
    ...
    if (!endOutOfRange){
        ...
        interpolationEngine->interpolateFromCouple (
            converter.fineDynamics( x1,y1,z1,
                                    delta[0].x, delta[0].y,delta[0].z).getDecomposedValues(whichTime), 0,1 );
    }
        ...
        if (!endOutOfRange){
            interpolationEngine->interpolateFromCouple (
                converter.fineDynamics( x1,y1,z1,
                                        delta[1].x,
                                        delta[1].y,
                                        delta[1].z ).getDecomposedValues(whichTime), 0,2 );
            interpolationEngine->interpolateFromAll(
                converter.fineDynamics( x1,y1,z1,
                                        delta[2].x,delta[2].y,delta[2].z ).getDecomposedValues(whichTime) );
        }
...
std::vector<Box3D> createCorners(Box3D domain, plint direction){
    ...
    if (direction == 0){ // the YZ plane
        result.push_back(Box3D(domain.x0,domain.x1,domain.y0,domain.y0,domain.z1,domain.z1)); // CUL
        result.push_back(Box3D(domain.x0,domain.x1,domain.y0,domain.y0,domain.z0,domain.z0)); // CLL
        result.push_back(Box3D(domain.x0,domain.x1,domain.y1,domain.y1,domain.z0,domain.z0)); // CLR
        result.push_back(Box3D(domain.x0,domain.x1,domain.y1,domain.y1,domain.z1,domain.z1)); // CUR
    }
...
Box3D createLeftDomain(Box3D originalDomain, plint direction){
    Box3D result;
    if (direction == 0){ // the YZ plane
        result = Box3D(originalDomain.x0,originalDomain.x1,
                       originalDomain.y0,originalDomain.y0,
                       originalDomain.z0,originalDomain.z1);
    }
    if (direction == 1){ // the XZ plane
        result = Box3D(originalDomain.x0,originalDomain.x0,
                       originalDomain.y0,originalDomain.y1,
                       originalDomain.z0,originalDomain.z1);
    }
    if (direction == 2){ // the XY plane
        result = Box3D(originalDomain.x0,originalDomain.x0,
...
Box3D createRightDomain(Box3D originalDomain, plint direction){
    ...
    if (direction == 0){ // the YZ plane
        result = Box3D(originalDomain.x0,originalDomain.x1,
                       originalDomain.y1,originalDomain.y1,
                       originalDomain.z0,originalDomain.z1);
    }
    if (direction == 1){ // the XZ plane
        result = Box3D(originalDomain.x1,originalDomain.x1,
                       originalDomain.y0,originalDomain.y1,
                       originalDomain.z0,originalDomain.z1);
    }
    if (direction == 2){ // the XY plane
...
InterpolateCoarseToFineDynamics3D<T,Descriptor1,Descriptor2>::InterpolateCoarseToFineDynamics3D (
        ...
        InterpolationEngine2D<T,Descriptor1>* interpolationEngine_,
        plint direction_, const Dot3D delta_[2] )
    : rescaleEngine(rescaleEngine_),
      interpolationEngine(interpolationEngine_),
...
InterpolateCoarseToFineDynamics3D<T,Descriptor1,Descriptor2>::InterpolateCoarseToFineDynamics3D (
    ...
    : rescaleEngine(rhs.rescaleEngine->clone()),
      ...
      direction(rhs.direction), delta(rhs.delta)
{ }
...
template<typename T, template<typename U> class Descriptor1, template<typename U> class Descriptor2>
    void InterpolateCoarseToFineDynamics3D<T,Descriptor1,Descriptor2>::process (
         Box3D coarseDomain,
         BlockLattice3D<T,Descriptor1>& coarseLattice,
         BlockLattice3D<T,Descriptor2>& fineLattice )
{
    ...
    PLB_PRECONDITION ( delta[0].x+delta[0].y+delta[0].z==1 && delta[1].x+delta[1].y+delta[1].z==1 );
    // creation of delta[2]
    delta.push_back(Dot3D(delta[0].x+delta[1].x,delta[0].y+delta[1].y,delta[0].z+delta[1].z));
    ...
    Dot3D fineLeftExcess   ( converter.fineX(x0) - delta[0].x,
                             converter.fineY(y0) - delta[0].y, 
                             converter.fineZ(z0) - delta[0].z );
    Dot3D coarseRightExcess( x1 + delta[0].x, y1 + delta[0].y, z1 + delta[0].z );
    Dot3D fineRightExcess  ( converter.fineX(x1) + delta[0].x,
                             converter.fineY(y1) + delta[0].y,
                             converter.fineZ(z1) + delta[0].z );
    Dot3D coarseBottomExcess ( x0 - delta[1].x, y0 - delta[1].y, z0 - delta[1].z );
    Dot3D fineBottomExcess   ( converter.fineX(x0) - delta[1].x,
                               converter.fineY(y0) - delta[1].y, 
                               converter.fineZ(z0) - delta[1].z );
    Dot3D coarseTopExcess( x1 + delta[1].x, y1 + delta[1].y, z1 + delta[1].z );
    Dot3D fineTopExcess  ( converter.fineX(x1) + delta[1].x,
                            converter.fineY(y1) + delta[1].y,
    ...
    bool bottomOutOfRange = ( !contained(coarseBottomExcess, coarseLattice.getBoundingBox()) ||
    ...
    bool topOutOfRange = ( !contained(coarseTopExcess, coarseLattice.getBoundingBox()) ||
    ...
    for (plint iX=x0; iX<=x1-delta[2].x; ++iX) {
        for (plint iY=y0; iY<=y1-delta[2].y; ++iY){
            for (plint iZ=z0; iZ<=z1-delta[2].z; ++iZ){
                ...
                interpolationEngine->pushToCouple (
                    Array<Cell<T,Descriptor1>,2>( coarseLattice.get(iX, iY, iZ),
                                                  coarseLattice.get(iX+delta[0].x,
                                                               iY+delta[0].y,
                                                               iZ+delta[0].z)),
                    *rescaleEngine );
                interpolationEngine->pushToCouple (
                    Array<Cell<T,Descriptor1>,2>(coarseLattice.get( iX+delta[1].x,
                                                                    iY+delta[1].y,
                                                                    iZ+delta[1].z),
                                                     coarseLattice.get( iX+delta[2].x,
                                                                        iY+delta[2].y,
                ...
                interpolationEngine->copyFrom (
                    converter.fineDynamics(iX,iY,iZ, 0,0,0).getDecomposedValues(whichTime), 0 );
                interpolationEngine->copyFrom (
                    converter.fineDynamics(iX+delta[0].x,iY+delta[0].y,iZ+delta[0].z,
                                           0,0,0).getDecomposedValues(whichTime), 1 );
                interpolationEngine->copyFrom (
                    converter.fineDynamics(iX+delta[1].x,iY+delta[1].y,iZ+delta[1].z,
                                           0,0,0).getDecomposedValues(whichTime), 2 );
                interpolationEngine->copyFrom (
                    converter.fineDynamics(iX+delta[2].x,iY+delta[2].y,iZ+delta[2].z,
                                           0,0,0).getDecomposedValues(whichTime), 3 );
                ...
                interpolationEngine->interpolateFromCouple (
                        converter.fineDynamics(iX+delta[0].x,iY+delta[0].y,iZ+delta[0].z,
                                               delta[1].x, delta[1].y,delta[1].z).getDecomposedValues(whichTime), 1,3 );
                interpolationEngine->interpolateFromCouple (
                        converter.fineDynamics(iX+delta[1].x,iY+delta[1].y,iZ+delta[1].z,
                                               delta[0].x, delta[0].y,delta[0].z).getDecomposedValues(whichTime), 2,3 );
    ...
    if (!leftOutOfRange){
        ...
        InterpolateOverLineAndExcess<T,Descriptor1>( createLeftDomain(coarseDomain,direction), 
                                                     coarseLattice, fineLattice,
                                                     interpolationEngine->clone(), rescaleEngine->clone(),
                                                     deltas );
    ...
    if (!bottomOutOfRange){
        ...
        deltas.push_back(Dot3D(delta[0].x,delta[0].y,delta[0].z));
        deltas.push_back(Dot3D(-delta[1].x,-delta[1].y,-delta[1].z));
        InterpolateOverLineAndExcess<T,Descriptor1>( createBottomDomain(coarseDomain,direction), 
                                                     coarseLattice, fineLattice,
                                                     interpolationEngine->clone(), rescaleEngine->clone(),
                                                     deltas );
    }
    ...
    if (!topOutOfRange){
        ...
        deltas.push_back(Dot3D(delta[0].x,delta[0].y,delta[0].z));
        deltas.push_back(Dot3D(delta[1].x,delta[1].y,delta[1].z));
        InterpolateOverLineAndExcess<T,Descriptor1>( createTopDomain(coarseDomain,direction), 
                                                     coarseLattice, fineLattice,
                                                     interpolationEngine->clone(), rescaleEngine->clone(),
                                                     deltas );
    }
    ...
    if (!rightOutOfRange){
        ...
        deltas.push_back(Dot3D(delta[1].x,delta[1].y,delta[1].z));
        deltas.push_back(Dot3D(delta[0].x,delta[0].y,delta[0].z));
        InterpolateOverLineAndExcess<T,Descriptor1>( createRightDomain(coarseDomain,direction), 
                                                     coarseLattice, fineLattice,
                                                     interpolationEngine->clone(), rescaleEngine->clone(),
                                                     deltas );
    }
    ...
    if (!leftOutOfRange && !topOutOfRange){
        const Dot3D deltas[2] = {Dot3D(delta[1].x,delta[1].y,delta[1].z),Dot3D(-delta[0].x,-delta[0].y,-delta[0].z)};
        applyProcessingFunctional(
            new InterpolateCoarseToFineCornerDynamics3D<T,Descriptor1,Descriptor2>(
                rescaleEngine->clone(), interpolationEngine->clone(),
                deltas
            ),
            corners[0],
            coarseLattice, fineLattice
...
template<typename T, template<typename U> class Descriptor1, template<typename U> class Descriptor2>
InterpolateCoarseToFineBoundaryDynamics3D<T,Descriptor1,Descriptor2>::InterpolateCoarseToFineBoundaryDynamics3D (
        RescaleEngine<T,Descriptor1>* rescaleEngine_,
        InterpolationEngine2D<T,Descriptor1>* interpolationEngine_,
...
InterpolateCoarseToFineBoundaryDynamics3D<T,Descriptor1,Descriptor2>::InterpolateCoarseToFineBoundaryDynamics3D (
    ...
    : rescaleEngine(rhs.rescaleEngine->clone()),
      interpolationEngine(rhs.interpolationEngine->clone()), delta(rhs.delta)
...
void InterpolateCoarseToFineBoundaryDynamics3D<T,Descriptor1,Descriptor2>::process (
         ...
         BlockLattice3D<T,Descriptor2>& fineLattice )
{
    InterpolateOverLineAndExcess<T,Descriptor1>(domain, coarseLattice, fineLattice,
                                                interpolationEngine->clone(), rescaleEngine->clone(),
                                                delta);
...
template<typename T, template<typename U> class Descriptor1, template<typename U> class Descriptor2>
InterpolateCoarseToFineCornerDynamics3D<T,Descriptor1,Descriptor2>::InterpolateCoarseToFineCornerDynamics3D (
        RescaleEngine<T,Descriptor1>* rescaleEngine_,
        InterpolationEngine2D<T,Descriptor1>* interpolationEngine_,
        const Dot3D delta_[2])
    : rescaleEngine(rescaleEngine_),
         ...
         BlockLattice3D<T,Descriptor2>& fineLattice )
{
    ...
    Dot3D fineExcess1   ( converter.fineX(x0) + delta[0].x,
                         converter.fineY(y0) + delta[0].y, 
                         converter.fineZ(z0) + delta[0].z );
    Dot3D coarseExcess2 ( x0 + delta[1].x, y0 + delta[1].y, z0 + delta[1].z );
    Dot3D fineExcess2   ( converter.fineX(x0) + delta[1].x,
                         converter.fineY(y0) + delta[1].y, 
                         converter.fineZ(z0) + delta[1].z );                     
    
    bool outOfRange1 = ( !contained( coarseExcess1, coarseLattice.getBoundingBox()) ||
                         !contained( fineExcess1  , fineLattice.getBoundingBox()) );
    bool outOfRange2 = ( !contained( coarseExcess2, coarseLattice.getBoundingBox()) ||
    ...
    if (!outOfRange1){
        interpolationEngine->pushToCouple (
                Array<Cell<T,Descriptor1>,2>(coarseLattice.get(x0, y0, z0),
                                             coarseLattice.get(x0+delta[0].x,
                                                               y0+delta[0].y,
                                                               z0+delta[0].z)),
                *rescaleEngine );
    }
    ...
    if (outOfRange2){
        ...
        interpolationEngine->pushToCouple (
                Array<Cell<T,Descriptor1>,2>(coarseLattice.get(x0+delta[1].x, y0+delta[1].y, z0+delta[1].z),
                                             coarseLattice.get(x0+delta[0].x+delta[1].x,
                                                               y0+delta[0].y+delta[1].y,
                                                               z0+delta[0].z+delta[1].z)),
                *rescaleEngine );
    }
    ...
    if (!outOfRange1) {
        interpolationEngine->interpolateFromCouple (
            converter.fineDynamics(x0,y0,z0, delta[0].x, delta[0].y, delta[0].z).getDecomposedValues(whichTime), 0,1 );
    }
    
    if (!outOfRange2) {
        interpolationEngine->interpolateFromCouple (
            converter.fineDynamics(x0,y0,z0, delta[1].x, delta[1].y, delta[1].z).getDecomposedValues(whichTime), 0,2 );
    }
    ...
    if (!outOfRange1 && !outOfRange2){
        interpolationEngine->interpolateFromAll(
            converter.fineDynamics(x0,y0,z0, 
                                   delta[1].x+delta[0].x,
                                   delta[1].y+delta[0].y,
                                   delta[1].z+delta[0].z).getDecomposedValues(whichTime) );
    }
...
template<typename T, template<typename U> class Descriptor>
void Copy_t1_to_t0_3D<T,Descriptor>::process(Box3D domain, BlockLattice3D<T,Descriptor>& fineLattice)
{
