...
template<typename T>
ScalarCopyFineToCoarseZerothOrder3D<T>& 
        ...
        Box3D fineDomain, ScalarField3D<T>& fineField, ScalarField3D<T>& coarseField )
{
    ...
    for (plint coarseX=coarseDomain.x0; coarseX<=coarseDomain.x1; ++coarseX, fineX+=stretch) {
        ...
        for (plint coarseY=coarseDomain.y0; coarseY<=coarseDomain.y1; ++coarseY, fineY+=stretch) {
            ...
            for (plint coarseZ=coarseDomain.z0; coarseZ<=coarseDomain.z1; ++coarseZ, fineZ+=stretch) {
...
template<typename T, int nDim>
TensorCopyFineToCoarseZerothOrder3D<T,nDim>* TensorCopyFineToCoarseZerothOrder3D<T,nDim>::clone() const{
...
void LatticeCopyFineToCoarseZerothOrder3D<T,Descriptor>::process (
        Box3D fineDomain,
        BlockLattice3D<T,Descriptor>& fineLattice, BlockLattice3D<T,Descriptor>& coarseLattice )
{
    ...
    for (plint coarseX=coarseDomain.x0; coarseX<=coarseDomain.x1; ++coarseX, fineX+=stretch) {
        ...
        for (plint coarseY=coarseDomain.y0; coarseY<=coarseDomain.y1; ++coarseY, fineY+=stretch) {
            ...
            for (plint coarseZ=coarseDomain.z0; coarseZ<=coarseDomain.z1; ++coarseZ, fineZ+=stretch) {
                ...
                // may not have the same number of variables in the decomposed vector.
...
template<typename T, template<typename U> class Descriptor>
LatticeCopyFineToCoarseZerothOrder3D<T,Descriptor>* LatticeCopyFineToCoarseZerothOrder3D<T,Descriptor>::clone() const{
...
template<typename T, template<typename U> class Descriptor>
LatticeCopyFineToCoarseNoScale3D<T,Descriptor>* LatticeCopyFineToCoarseNoScale3D<T,Descriptor>::clone() const{
...
template<typename T>
ScalarInterpolateCoarseToFine3D<T>& 
...
void ScalarInterpolateCoarseToFine3D<T>::process (
        Box3D coarseDomain,
        ScalarField3D<T>& coarseField, ScalarField3D<T>& fineField )
{
    Dot3D posFine = fineField.getLocation();
    Dot3D posCoarse = coarseField.getLocation();
            ...
            for (plint coarseZ=coarseDomain.z0; coarseZ<=coarseDomain.z1; ++coarseZ) {
                ...
                for (plint iZ=fineZ-stretch/2+1; iZ<=fineZ+stretch/2; ++iZ) {
                    if (contained(iX,iY,iZ, fineField.getBoundingBox())) {
                        ...
                        linearInterpolationCoefficients<T>(coarseField, Array<T,3> (
                                    coarseIx, coarseIy, coarseIz), cellPos, weights );
                        fineField.get(iX,iY,iZ) = 0.;
                        for (plint i=0; i<8; ++i) {
                            fineField.get(iX,iY,iZ) +=
                                weights[i] * coarseField.get(cellPos[i].x, cellPos[i].y, cellPos[i].z);
                        }
...
template<typename T, int nDim>
TensorInterpolateCoarseToFine3D<T,nDim>& 
...
void TensorInterpolateCoarseToFine3D<T,nDim>::process (
        Box3D coarseDomain,
        TensorField3D<T,nDim>& coarseField, TensorField3D<T,nDim>& fineField )
{
    Dot3D posFine = fineField.getLocation();
    Dot3D posCoarse = coarseField.getLocation();
            ...
            for (plint coarseZ=coarseDomain.z0; coarseZ<=coarseDomain.z1; ++coarseZ) {
                ...
                for (plint iZ=fineZ-stretch/2+1; iZ<=fineZ+stretch/2; ++iZ) {
                    if (contained(iX,iY,iZ, fineField.getBoundingBox())) {
                        ...
                        T coarseIy = (T)(iY + posFine.y)/(T)stretch;
                        T coarseIz = (T)(iZ + posFine.z)/(T)stretch;
                        linearInterpolationCoefficients(coarseField, Array<T,3> (
                                    coarseIx, coarseIy, coarseIz), cellPos, weights );
...
void LatticeInterpolateCoarseToFine3D<T,Descriptor>::process (
        Box3D coarseDomain,
        BlockLattice3D<T,Descriptor>& coarseLattice, BlockLattice3D<T,Descriptor>& fineLattice )
{
    static const int velDimDx =   1;
    static const int velDimDt =  -1;
    static const int presDimDx =  2;
            ...
            for (plint coarseZ=coarseDomain.z0; coarseZ<=coarseDomain.z1; ++coarseZ) {
                ...
                for (plint iZ=fineZ-stretch/2+1; iZ<=fineZ+stretch/2; ++iZ) {
                    if (contained(iX,iY,iZ, fineLattice.getBoundingBox())) {
                        ...
                        T coarseIy = (T)(iY + posFine.y)/(T)stretch;
                        T coarseIz = (T)(iZ + posFine.z)/(T)stretch;
                        linearInterpolationCoefficients(coarseLattice, Array<T,3> (
                                    coarseIx, coarseIy, coarseIz), cellPos, weights );
                        plint order = 0;
                        plint numComponents = 1+Descriptor<T>::d+Descriptor<T>::q+Descriptor<T>::ExternalField::numScalars;
                        // TODO: For now external scalars are set to zero during conversion. They should be
...
template<typename T, template<typename U> class Descriptor>
LatticeInterpolateCoarseToFine3D<T,Descriptor>* LatticeInterpolateCoarseToFine3D<T,Descriptor>::clone() const{
...
template<typename T, template<typename U> class Descriptor>
LatticeLinearInterpolateCoarseToFine3D<T,Descriptor>* LatticeLinearInterpolateCoarseToFine3D<T,Descriptor>::clone() const{
        ...
        plint dimDx, plint dimDt, plint dxScale, plint dtScale )
{
    ...
    MultiBlockManagement3D management = scaleMultiBlockManagement(fineField.getMultiBlockManagement(),relativeLevel);
