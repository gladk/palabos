...
MultiGridPeriodicitySwitch3D::MultiGridPeriodicitySwitch3D(MultiGridPeriodicitySwitch3D const& rhs)
    : block(rhs.block),periodicityArray(rhs.periodicityArray)
{ }

MultiGridPeriodicitySwitch3D& MultiGridPeriodicitySwitch3D::operator=(MultiGridPeriodicitySwitch3D const& rhs)
{
    block = rhs.block;
...
/// Copy
MultiGridStatSubscriber3D& MultiGridStatSubscriber3D::operator=(MultiGridStatSubscriber3D const& rhs)
{
...
// Constructor
MultiGrid3D::MultiGrid3D( MultiGridManagement3D management_, 
...
void MultiGrid3D::reduceStatistics(){
    ...
    for (plint iLevel=0; iLevel<this->getNumLevels(); ++iLevel) {
        ...
        levelStatistics[iLevel] =  new BlockStatistics(getComponent(iLevel).getInternalStatistics()); // copy
