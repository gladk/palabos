         ...
         RescaleEngine<T,Descriptor>* rescaleEngine)
{
    ...
    Dot3D startFineExcess ( converter.fineX(x0) - 2*delta.x,
                            converter.fineY(iY) - 2*delta.y,
    ...
    Dot3D endFineExcess ( converter.fineX(x1) + 2*delta.x,
                          converter.fineY(iY) + 2*delta.y,
                          converter.fineZ(iZ) + 2*delta.z );
    ...
    if (!startOutOfRange){
        rescaleEngine->scaleCoarseFine( coarseLattice.get(x0-2*delta.x,iY-2*delta.y,iZ-2*delta.z), pop1 );
        rescaleEngine->scaleCoarseFine( coarseLattice.get(x0-delta.x,iY-delta.y,iZ-delta.z)      , pop2 );
        rescaleEngine->scaleCoarseFine( coarseLattice.get(x0,iY,iZ)                              , pop3 );
        rescaleEngine->scaleCoarseFine( coarseLattice.get(x0+delta.x,iY+delta.y,iZ+delta.z)      , pop4 );
    ...
    for (plint iX=x0; iX<=x1-delta.x; ++iX){
        rescaleEngine->scaleCoarseFine( coarseLattice.get(iX-delta.x,iY-delta.y,iZ-delta.z), pop1 );
        rescaleEngine->scaleCoarseFine( coarseLattice.get(iX,iY,iZ), pop2 );
        rescaleEngine->scaleCoarseFine( coarseLattice.get(iX+delta.x,iY+delta.y,iZ+delta.z), pop3 );
        rescaleEngine->scaleCoarseFine( coarseLattice.get(iX+2*delta.x,iY+2*delta.y,iZ+2*delta.z), pop4 );
        
        // cubicInterpolation the unknown population
        cubicCenteredInterpolation<T>( pop1,pop2,pop3,pop4,
                        converter.fineDynamics(Dot3D(iX,iY,iZ),delta
                        ).getDecomposedValues(whichTime) );
    }
    
    // check if interpolation is needed in the rightmost part
    if (!endOutOfRange){
        rescaleEngine->scaleCoarseFine( coarseLattice.get(x1-delta.x,iY-delta.y,iZ-delta.z), pop1 );
        rescaleEngine->scaleCoarseFine( coarseLattice.get(x1,iY,iZ), pop2 );
        rescaleEngine->scaleCoarseFine( coarseLattice.get(x1+delta.x,iY+delta.y,iZ+delta.z), pop3 );
        rescaleEngine->scaleCoarseFine( coarseLattice.get(x1+2*delta.x,iY+2*delta.y,iZ+2*delta.z), pop4 );
        
        cubicCenteredInterpolation<T>( pop1,pop2,pop3,pop4,
                        converter.fineDynamics(Dot3D(x1,iY,iZ),delta
                        ).getDecomposedValues(whichTime) );
    }
         ...
         plint orientation )
{
    ...
    Box3D line(converter.fineX(x0),converter.fineX(x1),
               converter.fineY(iY)+orientation,converter.fineY(iY)+orientation,
               converter.fineZ(iZ),converter.fineZ(iZ));
         ...
         RescaleEngine<T,Descriptor>* rescaleEngine)
{
    ...
    Dot3D startFineExcess ( converter.fineX(iX) - 2*delta.x,
                            converter.fineY(y0) - 2*delta.y,
                            converter.fineZ(iZ) - 2*delta.z );
    bool startOutOfRange = ( !contained(startExcess, coarseLattice.getBoundingBox()) ||
                             !contained(startFineExcess, fineLattice.getBoundingBox()) );
                             
    Dot3D endExcess ( iX + 2*delta.x, y1 + 2*delta.y, iZ + delta.z);
    Dot3D endFineExcess ( converter.fineX(iX) + 2*delta.x,
                          converter.fineY(y1) + 2*delta.y,
                          converter.fineZ(iZ) + 2*delta.z );
    bool endOutOfRange = ( !contained(endExcess, coarseLattice.getBoundingBox()) ||
                           !contained(endFineExcess, fineLattice.getBoundingBox()) );
    ...
    // check if interpolation is needed in the leftmost part
    if (!startOutOfRange){
        rescaleEngine->scaleCoarseFine( coarseLattice.get(iX-2*delta.x,y0-2*delta.y,iZ-2*delta.z), pop1 );
        rescaleEngine->scaleCoarseFine( coarseLattice.get(iX-delta.x,y0-delta.y,iZ-delta.z)      , pop2 );
        rescaleEngine->scaleCoarseFine( coarseLattice.get(iX,y0,iZ)                              , pop3 );
        rescaleEngine->scaleCoarseFine( coarseLattice.get(iX+delta.x,y0+delta.y,iZ+delta.z)      , pop4 );
    ...
    // check if interpolation is needed in the rightmost part
    if (!endOutOfRange){
        rescaleEngine->scaleCoarseFine( coarseLattice.get(iX-delta.x,y1-delta.y,iZ-delta.z), pop1 );
        rescaleEngine->scaleCoarseFine( coarseLattice.get(iX,y1,iZ), pop2 );
        rescaleEngine->scaleCoarseFine( coarseLattice.get(iX+delta.x,y1+delta.y,iZ+delta.z), pop3 );
        rescaleEngine->scaleCoarseFine( coarseLattice.get(iX+2*delta.x,y1+2*delta.y,iZ+2*delta.z), pop4 );
        
        cubicCenteredInterpolation<T>( pop1,pop2,pop3,pop4,
                        converter.fineDynamics(Dot3D(iX,y1,iZ),delta
                        ).getDecomposedValues(whichTime) );
    }
}
         ...
         RescaleEngine<T,Descriptor>* rescaleEngine)
{
    ...
    Box3D coarseDomain (
                domain.shift(posFine.x,posFine.y,posFine.z).  // Convert to absolute fine coordinates.
