...
template<typename T, class Descriptor> struct externalForceTemplatesImpl;
...
{
    externalForceTemplatesImpl<T, Descriptor<T> >
        ::addNaiveForce(cell.getRawPopulations(), cell.getExternal(0));
}
...
static void addGuoCompressibleForce( Cell<T,Descriptor>& cell, T rho, Array<T,Descriptor<T>::d> const& u,
                                     T uSqr, T thetaBar, T omega, T amplitude)
{
    externalForceTemplatesImpl<T, Descriptor<T> >
        ::addGuoCompressibleForce(cell.getRawPopulations(), cell.getExternal(0), rho, u, uSqr, thetaBar, omega, amplitude);
}
...
{
    return externalForceTemplatesImpl<T, Descriptor<T> >
               ::shanChenForcedBGKCollision(cell.getRawPopulations(), cell.getExternal(0),
                                            rhoBar, j, omega);
}
...
{
    return externalForceTemplatesImpl<T, Descriptor<T> >
               ::heForcedBGKCollision(cell.getRawPopulations(), cell.getExternal(0),
                                            rhoBar, j, omega);
}
...
static void addGuoForce( Array<T,Descriptor::q>& f, T* externalScalars,
...
static void addGuoCompressibleForce( Array<T,Descriptor::q>& f, T* externalScalars,
                                     T rho, Array<T,Descriptor::d> const& u, T uSqr, T thetaBar,
                                     T omega, T amplitude )
{
    static const int forceBeginsAt = Descriptor::ExternalField::forceBeginsAt;
    T* force = externalScalars + forceBeginsAt;
    for (plint iPop=0; iPop < Descriptor::q; ++iPop) 
    {
        ...
        forceTerm += Descriptor::invCs2*Descriptor::invCs2/(T)2 * (c_u*c_f-Descriptor::cs2*f_u);// Hermite order 2
        forceTerm +=  Descriptor::invCs2*Descriptor::invCs2*Descriptor::invCs2/(T)6 *
            (c_f*(c_u*c_u+Descriptor::cs2*(Descriptor::cNormSqr[iPop]*thetaBar))
            - Descriptor::cs2*(c_u*((T)2*f_u + uSqr+Descriptor::cs2*Descriptor::d*thetaBar) + (T)2*Descriptor::cs2*thetaBar*c_f));// Hermite order 3
...
static T heForcedBGKCollision (
        Array<T,Descriptor::q>& f, T* externalScalars,
        T rhoBar, Array<T,Descriptor::d> const& j, T omega )
{
    static const int forceBeginsAt = Descriptor::ExternalField::forceBeginsAt;
    T* force = externalScalars + forceBeginsAt;
