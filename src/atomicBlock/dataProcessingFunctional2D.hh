...
{
    process( domain,
             dynamic_cast<NTensorField2D<T>&>(*atomicBlocks[0]),
             dynamic_cast<NTensorField2D<int>&>(*atomicBlocks[1]) );
...
{
    process ( domain,
              dynamic_cast<ScalarField2D<T1>&>(*atomicBlocks[0]),
...
{
    process ( domain,
              dynamic_cast<TensorField2D<T1,nDim1>&>(*atomicBlocks[0]),
...
{
    process ( domain,
              dynamic_cast<NTensorField2D<T1>&>(*atomicBlocks[0]),
        ...
        DotList2D const& dotList, std::vector<AtomicBlock2D*> atomicBlocks )
{
    std::vector<BlockLattice2D<T,Descriptor>*> lattices(atomicBlocks.size());
    for (pluint iLattice=0; iLattice<atomicBlocks.size(); ++iLattice) {
        lattices[iLattice] = dynamic_cast<BlockLattice2D<T,Descriptor>*> (
...
{
    processEdge(direction, orientation, domain,
                dynamic_cast<TensorField2D<T,nDim>&>(*atomicBlocks[0]));
}
...
template<typename T1, template<typename U1> class Descriptor1,
         typename T2, template<typename U2> class Descriptor2>
void BoundedBoxProcessingFunctional2D_LL<T1,Descriptor1,T2,Descriptor2>::
...
template<typename T1, template<typename U1> class Descriptor1,
         typename T2, template<typename U2> class Descriptor2>
void BoundedBoxProcessingFunctional2D_LL<T1,Descriptor1,T2,Descriptor2>::
    processEdgeGeneric( int direction, int orientation,
                        Box2D domain, std::vector<AtomicBlock2D*> atomicBlocks )
{
    PLB_PRECONDITION( atomicBlocks.size() == 2 );
    processEdge( direction, orientation, domain,
...
template<typename T1, template<typename U1> class Descriptor1,
         typename T2, template<typename U2> class Descriptor2>
void BoundedBoxProcessingFunctional2D_LL<T1,Descriptor1,T2,Descriptor2>::
    processCornerGeneric( int normalX, int normalY,
                          Box2D domain, std::vector<AtomicBlock2D*> atomicBlocks )
{
    PLB_PRECONDITION( atomicBlocks.size() == 2 );
    processCorner( normalX, normalY, domain,
