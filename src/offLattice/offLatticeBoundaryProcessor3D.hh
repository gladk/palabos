...
template<typename T>
CheckVoxelizationFunctional3D<T>::
    CheckVoxelizationFunctional3D (
...
template<typename T>
CheckVoxelizationFunctional3D<T>&
    CheckVoxelizationFunctional3D<T>::operator= (
            CheckVoxelizationFunctional3D<T> const& rhs )
{
...
template<typename T>
CheckVoxelizationFunctional3D<T>*
    CheckVoxelizationFunctional3D<T>::clone() const
{
        ...
        Box3D domain, std::vector<AtomicBlock3D*> fields )
{
    ...
    ScalarField3D<int>* flagMatrix =
        dynamic_cast<ScalarField3D<int>*>(fields[0]);
    ...
    if (!isFluid(cellLocation+offset)) {
        ...
        for (int iNeighbor=0; iNeighbor<NextNeighbor<T>::numNeighbors; ++iNeighbor) {
            ...
            if (isFluid(neighbor+offset)) {
                ...
                bool ok = shape->pointOnSurface(cellLocation+offset, Dot3D(c[0],c[1],c[2]),
