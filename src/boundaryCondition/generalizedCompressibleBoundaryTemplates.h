...
struct generalizedComprTempBoundaryTemplates {
    ...
    static T equilibriumOverRho(plint iPop, const Array<T,Descriptor<T>::d> &u, T uSqr, T thetaBar) {
        ...
        return L::t[iPop] * (
            (T)1 + L::invCs2 * c_u +
            L::invCs2/(T)2 * ((L::invCs2 * c_u*c_u - uSqr )
                                        + thetaBar * (L::cNormSqr[iPop]-L::cs2*L::d))
            + L::invCs2*L::invCs2*L::invCs2/(T)6 * c_u * (
                c_u*c_u-(T)3*L::cs2*uSqr+(T)3*L::cs2*thetaBar*(L::cNormSqr[iPop]-L::cs2*(L::d+2))
            )
            + L::invCs2*L::invCs2*L::invCs2*L::invCs2/(T)24 * (
                c_u*c_u*c_u*c_u - (T)6*L::cs2*uSqr*c_u*c_u + (T)3*L::cs2*L::cs2*uSqr*uSqr
                + (T)6*L::cs2*thetaBar*(c_u*c_u*(L::cNormSqr[iPop]-L::cs2*(L::d+4))+L::cs2*uSqr*(L::cs2*(L::d+2)-L::cNormSqr[iPop]))
                + (T)3*L::cs2*L::cs2*thetaBar*thetaBar*(
                    L::cNormSqr[iPop]*L::cNormSqr[iPop]-(T)2*L::cs2*(L::d+2)*L::cNormSqr[iPop]+L::cs2*L::cs2*L::d*(L::d+2)
                )
            )
        );
    }
...
// ================================================================================== //
// ================================================================================== //
    static void computeDiffF(plint iPop, T rho, const Array<T,Descriptor<T>::d> &u, T uSqr,
                             T thetaBar, Eigen::RowVectorXd &df) {
        ...
        Array<T,SymmetricRankThreeTensor<T,Descriptor>::n> H3 = HermiteTemplate<T,Descriptor>::order3(iPop);
        Array<T,SymmetricRankFourTensor<T,Descriptor>::n> H4 = HermiteTemplate<T,Descriptor>::order4(iPop);
        ...
        df(1+Descriptor<T>::d) += tcs4/(T)6 * VectorTemplate<T,Descriptor>::scalarProduct(H3contracted, u);
        df(1+Descriptor<T>::d) += tcs6_2/(T)2 * (SymmetricTensor<T,Descriptor>::contractIndexes(H4contracted, u2)
        ...
        for (plint iPi = 0; iPi < SymmetricRankThreeTensor<T,Descriptor>::n; ++iPi) {
            df(1+Descriptor<T>::d+1+SymmetricTensor<T,Descriptor>::n+iPi) = H3[iPi]*tcs6_2/(T)3;
        }

    }
    
    static void computeDiffFtrLessPiNeq(plint iPop, T rho, const Array<T,Descriptor<T>::d> &u, T uSqr,
                                        T thetaBar, Eigen::RowVectorXd &df) {
        df(0) = equilibriumOverRho(iPop, u, uSqr, thetaBar); // df/drho
        ...
        H2 = HermiteTemplate<T,Descriptor>::contractedOrder2(iPop);
        df(2+Descriptor<T>::d) = (H2[SymmetricTensorImpl<T,2>::xx]-H2[SymmetricTensorImpl<T,2>::yy])*tcs4/(T)2;
        df(2+Descriptor<T>::d+1) = H2[SymmetricTensorImpl<T,2>::xy]*tcs4/(T)2;
        H3 = HermiteTemplate<T,Descriptor>::contractedOrder3(iPop);
        for (plint iPi = 0; iPi < SymmetricRankThreeTensor<T,Descriptor>::n; ++iPi) {
            df(1+Descriptor<T>::d+SymmetricTensor<T,Descriptor>::n+iPi) = H3[iPi]*tcs6_2/(T)3;
        }

    }

    static void computeJacobian(T rho, const Array<T,Descriptor<T>::d> &u,
                                T thetaBar, const std::vector<plint> &knownIndices, Eigen::MatrixXd &Jac) {
        plint systSizeX =  SymmetricRankThreeTensor<T,Descriptor>::n
                           + SymmetricTensor<T,Descriptor>::n
                           + 1 + Descriptor<T>::d + 1;
    ...
    static void computeJacobianTrLessPiNeq(T rho, const Array<T,Descriptor<T>::d> &u,
                                           T thetaBar, const std::vector<plint> &knownIndices, Eigen::MatrixXd &Jac) {
        plint systSizeX =  SymmetricRankThreeTensor<T,Descriptor>::n
                           + SymmetricTensor<T,Descriptor>::n
                           + 1 + Descriptor<T>::d;
    ...
    static void computeNonLinearFunction(const Cell<T,Descriptor>& cell, 
        ...
        for (pluint iPop = 0; iPop < knownIndices.size(); ++iPop) {
            f(iPop) = cell[knownIndices[iPop]] - (
                dynamicsTemplates<T,Descriptor>::bgk_ma4_equilibrium(knownIndices[iPop], rhoBar, invRho, j, jSqr, thetaBar) +
                offEquilibriumTemplates<T,Descriptor>::fromPiAndQtoFneq(knownIndices[iPop],PiNeq, Qneq) );
...
//         pcout << std::endl;
//         pcout << "num = " << u[dir] << ", " << PiNeq[0] << ", " << PiNeq[1] << ", " << PiNeq[2] << std::endl;
        return true;
    }
    
    static bool convergeTrLessPiNeq(Eigen::VectorXd &x,
                         const T rho,
                         const Array<T,Descriptor<T>::d> &u, 
                         const T thetaBar,
                         const Array<T,SymmetricTensor<T,Descriptor>::n> &PiNeq,
...
//         pcout << "num = " << u[dir] << ", " << PiNeq[0] << ", " << PiNeq[1] << ", " << PiNeq[2] << std::endl;
    ...
    static bool iterativelySolveSystem(const Cell<T,Descriptor>& cell, 
                                       T &rho,
        ...
        for (plint iT = 0; iT < maxT; ++iT) {
            T uSqr = VectorTemplate<T,Descriptor>::normSqr(u);
            computeNonLinearFunction(cell,rho,u,uSqr,thetaBar,PiNeq,Qneq,knownIndices,missingIndices,f);
...
//             Jac.lu().solve(f,&x);
            ...
            for (plint iPi = 0; iPi < SymmetricRankThreeTensor<T,Descriptor>::n; ++iPi) {
                Qneq[iPi] += stepMult*x(iPi+2+Descriptor<T>::d+SymmetricTensor<T,Descriptor>::n);
            }
...
//         pcout << "NEVER CONVERGED!!!." << std::endl;
    ...
    static bool iterativelySolveSystemTrLessPiNeq(const Cell<T,Descriptor>& cell, 
                                       T &rho,
                                       Array<T,Descriptor<T>::d> &u,
                                       T &thetaBar,
                                       Array<T,SymmetricTensor<T,Descriptor>::n> &PiNeq,
...
//             Jac.lu().solve(f,&x);
            ...
            for (plint iPi = 0; iPi < SymmetricRankThreeTensor<T,Descriptor>::n; ++iPi) {
                Qneq[iPi] += stepMult*x(iPi+1+Descriptor<T>::d+SymmetricTensor<T,Descriptor>::n);
            }
...
//         pcout << "NEVER CONVERGED!!!." << std::endl;
    ...
    static void computeMatrixRow(
        plint iPop, const Array<T,Descriptor<T>::d> &u, T uSqr, 
        T thetaBar, T omega, Eigen::RowVectorXd &a) {
        
        T eqOverRho = equilibriumOverRho(iPop,u,uSqr,thetaBar);
        a(0) = eqOverRho;
        
        T oneMinusOmega = (T)1-omega;
        T factor = oneMinusOmega * Descriptor<T>::t[iPop]*Descriptor<T>::invCs2*Descriptor<T>::invCs2 / (T)2;
        ...
        Array<T,SymmetricRankThreeTensor<T,Descriptor>::n> H3 = HermiteTemplate<T,Descriptor>::contractedOrder3(iPop);
        for (plint iPi=0; iPi < SymmetricRankThreeTensor<T,Descriptor>::n; ++iPi) {
    ...
    static void computeMatrixRow(
            plint iPop, const Array<T,Descriptor<T>::d> &u, T uSqr, 
            T thetaBar, Eigen::RowVectorXd &a) {
    ...
    static void createLinearSystem(
        ...
        plint systSizeX = SymmetricRankThreeTensor<T,Descriptor>::n+SymmetricTensor<T,Descriptor>::n+1;
//         plint systSizeX = SymmetricRankThreeTensor<T,Descriptor>::n+SymmetricTensor<T,Descriptor>::n; // PiNeq is traceless...
...
//             pcout << ", uSqr = " << uSqr << ", thetaBar = " << thetaBar << ", omega = " << omega;
...
//         pcout << ", rhoIn = " << rhoIn << std::endl;
    ...
    static void solveLinearSystem(const Cell<T,Descriptor>& cell, 
                                  const Array<T,Descriptor<T>::d> &u,
                                  T thetaBar,
                                  const std::vector<plint> &missingIndices,
                                  const std::vector<plint> &knownIndices,
...
//         pcout << "TrPiNeq = " << PiNeq[0] - PiNeq[2] << ", PiNeq[xx] = " << PiNeq[0] << ", PiNeq[xy] = " << PiNeq[1] << ", PiNeq[yy] = " << PiNeq[2] << std::endl;
    }
    ...
    static void solveLinearSystemTrLessPiNeq(
            const Cell<T,Descriptor>& cell, const Array<T,Descriptor<T>::d> &u, T thetaBar,
            const std::vector<plint> &missingIndices, const std::vector<plint> &knownIndices,
            T &rho, Array<T,SymmetricTensor<T,Descriptor>::n> &PiNeq, 
            Array<T,SymmetricRankThreeTensor<T,Descriptor>::n> &qNeq,bool massConservation) {
        Eigen::MatrixXd A;
        Eigen::VectorXd b;
        
        createLinearSystemTrLessPiNeq(cell, u, thetaBar, missingIndices, knownIndices, A, b, massConservation);
...
//         pcout << "TrPiNeq = " << PiNeq[0] - PiNeq[2] << ", PiNeq[xx] = " << PiNeq[0] << ", PiNeq[xy] = " << PiNeq[1] << ", PiNeq[yy] = " << PiNeq[2] << std::endl;
    ...
    // Matrix row corespond to the factors in front of each Hermite coefficient (the Hermite polynomials times the weights).
    // a(0) = w_i H0, a(1,..,d) = w_i/c_s^ 2*H1_ii, a(d+1,..,SymRank2::n)=w_i/(2c_s^4)Q_i
    // a(SymRank2::n+1,..,SymRank3::n) = w_i, a(1,..,d) = w_i/(6c_s^6)*H3_i
    // a(SmyRank3::n+1,...,SmyRank4::n)=w_i/(24c_s^8)H4_i
    static void computeMatrixRow(plint iPop, Eigen::RowVectorXd &a) {
        T factor = Descriptor<T>::t[iPop];
        a(0) = (T)1*factor;

        factor *= Descriptor<T>::invCs2;
        for (plint iPi=0; iPi < Descriptor<T>::d; ++iPi) a(iPi+1) = Descriptor<T>::c[iPop][iPi] * factor;

        factor *= Descriptor<T>::invCs2 / (T)2;
        Array<T,SymmetricTensor<T,Descriptor>::n> H2 = HermiteTemplate<T,Descriptor>::contractedOrder2(iPop);
        for (plint iPi = 0; iPi < SymmetricTensor<T,Descriptor>::n; ++iPi) a(iPi+1+Descriptor<T>::d) = H2[iPi]*factor;
        ...
        Array<T,SymmetricRankFourTensor<T,Descriptor>::n> H4 = HermiteTemplate<T,Descriptor>::contractedOrder4(iPop);

        for (plint iPi=0; iPi < SymmetricRankFourTensor<T,Descriptor>::n; ++iPi) {
            a(iPi+1+Descriptor<T>::d+SymmetricTensor<T,Descriptor>::n+SymmetricRankThreeTensor<T,Descriptor>::n)
                = H4[iPi]*factor;
        }
    }
    ...
    static void createLinearSystem(const Cell<T,Descriptor>& cell,
                                   const std::vector<plint> &missingIndices,
                                   const std::vector<plint> &knownIndices,
...
//             pcout << "a3 = " << coeffs[iPi+1+Descriptor<T>::d+SymmetricTensor<T,Descriptor>::n] << ", ";
        }
        for (plint iPi = 0; iPi < SymmetricRankFourTensor<T,Descriptor>::n; ++iPi) {
            coeffs.push_back(x(iPi+1+Descriptor<T>::d+SymmetricTensor<T,Descriptor>::n+SymmetricRankThreeTensor<T,Descriptor>::n));
//             pcout << "a4 = " << coeffs[iPi+1+Descriptor<T>::d+SymmetricTensor<T,Descriptor>::n+SymmetricRankThreeTensor<T,Descriptor>::n] << ", ";
        }
//         pcout << std::endl;
    }
