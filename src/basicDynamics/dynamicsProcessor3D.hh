        ...
        NTensorField3D<T> const& rhoBarJfield, Dot3D const& offset, BlockStatistics& stat )
{
    ...
    for (plint iX=domain.x0; iX<=domain.x1; ++iX) {
        for (plint iY=domain.y0; iY<=domain.y1; ++iY) {
            for (plint iZ=domain.z0; iZ<=domain.z1; ++iZ) {
                Cell<T,Descriptor>& cell = lattice.get(iX,iY,iZ);
                T const* macroscopic = rhoBarJfield.get(iX+offset.x, iY+offset.y, iZ+offset.z);
...
template<typename T, template<typename U> class Descriptor>
MaskedCollide3D<T,Descriptor>& MaskedCollide3D<T,Descriptor>::operator=(MaskedCollide3D<T,Descriptor> const& rhs)
{
...
template<typename T, template<typename U> class Descriptor>
void MaskedCollide3D<T,Descriptor>::getTypeOfModification(std::vector<modif::ModifT>& modified) const
{
    modified[0] = modif::staticVariables;
    modified[1] = modif::nothing;
}
...
{
    applyProcessingFunctional (
            new MaskedCollide3D<T,Descriptor>(dynamics, flag),
            lattice.getBoundingBox(), lattice, mask );
}
