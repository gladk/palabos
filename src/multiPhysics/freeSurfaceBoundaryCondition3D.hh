        ...
        for (plint iY=domain.y0; iY<=domain.y1; ++iY) {
            ...
            for (plint iZ=domain.z0; iZ<=domain.z1; ++iZ) {
                ...
                if (numSpongeCells[0] > 0 && x <= spongePositions[0]) {
                    if (useTanhSpongeFunction) {
                        spongeFunction *= (T) 0.5 * ((T) 1 + std::tanh((T) (x - spongeCenters[0]) * invSigma[0]));
                    } else {
                        spongeFunction *= (T) 0.5 * ((T) 1 - std::cos(pi * (T) x * invNumSpongeCells[0]));
                    }
                    modify = true;
                }
                if (numSpongeCells[1] > 0 && x >= spongePositions[1]) {
                    if (useTanhSpongeFunction) {
                        spongeFunction *= (T) 0.5 * ((T) 1 - std::tanh((T) (x - spongeCenters[1]) * invSigma[1]));
                    } else {
                        spongeFunction *= (T) 0.5 * ((T) 1 + std::cos(pi * (T) (x - spongePositions[1]) * invNumSpongeCells[1]));
                    }
                    modify = true;
                }
                ...
                if (numSpongeCells[2] > 0 && y <= spongePositions[2]) {
                    if (useTanhSpongeFunction) {
                        spongeFunction *= (T) 0.5 * ((T) 1 + std::tanh((T) (y - spongeCenters[2]) * invSigma[2]));
                    } else {
                        spongeFunction *= (T) 0.5 * ((T) 1 - std::cos(pi * (T) y * invNumSpongeCells[2]));
                    }
                    modify = true;
                }
                if (numSpongeCells[3] > 0 && y >= spongePositions[3]) {
                    if (useTanhSpongeFunction) {
                        spongeFunction *= (T) 0.5 * ((T) 1 - std::tanh((T) (y - spongeCenters[3]) * invSigma[3]));
                    } else {
                        spongeFunction *= (T) 0.5 * ((T) 1 + std::cos(pi * (T) (y - spongePositions[3]) * invNumSpongeCells[3]));
                    }
                    modify = true;
                }
                ...
                if (numSpongeCells[4] > 0 && z <= spongePositions[4]) {
                    if (useTanhSpongeFunction) {
                        spongeFunction *= (T) 0.5 * ((T) 1 + std::tanh((T) (z - spongeCenters[4]) * invSigma[4]));
                    } else {
                        spongeFunction *= (T) 0.5 * ((T) 1 - std::cos(pi * (T) z * invNumSpongeCells[4]));
                    }
                    modify = true;
                }
                if (numSpongeCells[5] > 0 && z >= spongePositions[5]) {
                    if (useTanhSpongeFunction) {
                        spongeFunction *= (T) 0.5 * ((T) 1 - std::tanh((T) (z - spongeCenters[5]) * invSigma[5]));
                    } else {
                        spongeFunction *= (T) 0.5 * ((T) 1 + std::cos(pi * (T) (z - spongePositions[5]) * invNumSpongeCells[5]));
                    }
                    modify = true;
                }
