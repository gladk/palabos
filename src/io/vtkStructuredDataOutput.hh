...
template<typename T>
void VtkStructuredWriter3D::writeDataField(DataSerializer const* serializer,
...
VtkStructuredImageOutput2D<T>::VtkStructuredImageOutput2D(std::string fName, T deltaX_)
    : fullName ( global::directories().getVtkOutDir() + fName+".vts" ),
        vtkOut( fullName ), deltaX(deltaX_), offset(T(),T()), headerWritten( false )
{ }

template<typename T>
VtkStructuredImageOutput2D<T>::VtkStructuredImageOutput2D(std::string fName, T deltaX_, Array<T,2> offset_)
: fullName ( global::directories().getVtkOutDir() + fName+".vts" ),
    vtkOut( fullName ),
    deltaX(deltaX_),
    offset(offset_),
...
void VtkStructuredImageOutput2D<T>::writeHeader(plint nx_, plint ny_) {
    if (headerWritten) {
        ...
        vtkOut.startPiece(Box3D(0,nx-1,0,ny-1,0,0), Array<T,3>(offset[0],offset[1],T()), deltaX);
        headerWritten = true;
    }
}
...
void VtkStructuredImageOutput2D<T>::writeData( ScalarField2D<T> & scalarField,
...
template<typename T>
template<typename TConv>
void VtkStructuredImageOutput2D<T>::writeData( MultiScalarField2D<T> & scalarField,
                                    std::string scalarFieldName, TConv scalingFactor,
                                    TConv additiveOffset )
{
    writeHeader(scalarField.getNx(), scalarField.getNy());
...
void VtkStructuredImageOutput2D<T>::writeData( TensorField2D<T,n> & tensorField,
...
template<typename T>
template<plint n, typename TConv>
void VtkStructuredImageOutput2D<T>::writeData( MultiTensorField2D<T,n> & tensorField,
                                    std::string tensorFieldName, TConv scalingFactor )
{
    writeHeader(tensorField.getNx(), tensorField.getNy());
...
VtkStructuredImageOutput3D<T>::VtkStructuredImageOutput3D(std::string fName, T deltaX_)
    : fullName ( global::directories().getVtkOutDir() + fName+".vts" ),
        vtkOut( fullName ), deltaX(deltaX_), offset(T(),T(),T()), headerWritten( false )
{ }

template<typename T>
VtkStructuredImageOutput3D<T>::VtkStructuredImageOutput3D(std::string fName, T deltaX_, Array<T,3> offset_)
    : fullName ( global::directories().getVtkOutDir() + fName+".vts" ),
        vtkOut( fullName ), deltaX(deltaX_), offset(offset_), headerWritten( false )
{ }
...
template<typename T>
template<typename TConv>
void VtkStructuredImageOutput3D<T>::writeData( ScalarField3D<T> & scalarField,
                                    std::string scalarFieldName, TConv scalingFactor,
                                    TConv additiveOffset )
{
...
template<typename T>
template<plint n, typename TConv>
void VtkStructuredImageOutput3D<T>::writeData( TensorField3D<T,n> & tensorField,
                                    std::string tensorFieldName, TConv scalingFactor )
{
