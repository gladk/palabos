...
ParallelDynamics<T,Descriptor>::ParallelDynamics(std::vector<Cell<T,Descriptor>*>& baseCells_, bool hasBulkCell_)
...
template<typename T, template<typename U> class Descriptor>
void ParallelDynamics<T,Descriptor>::decompose(Cell<T,Descriptor> const& cell, std::vector<T>& rawData, plint order) const
{
    if (hasBulkCell) {
...
template<typename T, template<typename U> class Descriptor>
void ParallelDynamics<T,Descriptor>::recompose(Cell<T,Descriptor>& cell, std::vector<T> const& rawData, plint order) const
{
    for (pluint iCell=0; iCell<baseCells.size(); ++iCell) {
...
template<typename T, template<typename U> class Descriptor>
void ParallelDynamics<T,Descriptor>::rescale(std::vector<T>& rawData, T xDxInv, T xDt, plint order) const
{
    if (hasBulkCell) {
...
template<typename T, template<typename U> class Descriptor>
ConstParallelDynamics<T,Descriptor>::ConstParallelDynamics(std::vector<Cell<T,Descriptor> const*>& baseCells_, bool hasBulkCell_)
    : baseCells(baseCells_), hasBulkCell(hasBulkCell_)
{ }
...
template<typename T, template<typename U> class Descriptor>
void ConstParallelDynamics<T,Descriptor>::decompose(Cell<T,Descriptor> const& cell, std::vector<T>& rawData, plint order) const
{
    if (hasBulkCell) {
        baseCells[0] -> getDynamics().decompose(cell, rawData, order);
    }
...
template<typename T, template<typename U> class Descriptor>
void ConstParallelDynamics<T,Descriptor>::recompose(Cell<T,Descriptor>& cell, std::vector<T> const& rawData, plint order) const
{
    for (pluint iCell=0; iCell<baseCells.size(); ++iCell) {
        baseCells[iCell] -> getDynamics().recompose(cell, rawData, order);
    }
...
template<typename T, template<typename U> class Descriptor>
void ConstParallelDynamics<T,Descriptor>::rescale(std::vector<T>& rawData, T xDxInv, T xDt, plint order) const
{
    if (hasBulkCell) {
        baseCells[0] -> getDynamics().rescale(rawData, xDxInv, xDt, order);
    }
