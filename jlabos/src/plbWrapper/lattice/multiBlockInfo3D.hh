...
#ifndef SWIG_MULTI_BLOCK_LATTICE_INFO_3D_HH
#define SWIG_MULTI_BLOCK_LATTICE_INFO_3D_HH

#include "plbWrapper/lattice/multiBlockInfo3D.h"

namespace plb {

template<typename T, template<typename T> class Descriptor>
MultiBlockLatticeInfo<T,Descriptor>::MultiBlockLatticeInfo (
        MultiBlockLattice3D<T,Descriptor> const& multiBlock )
    : nx(), ny(), nz(),
      numBlocks(),
      numAllocatedCells()
{
...
template<typename T, template<typename T> class Descriptor>
plint MultiBlockLatticeInfo<T,Descriptor>::getNz() const {
    return nz;
}
...
template<typename T, template<typename T> class Descriptor>
Box3D MultiBlockLatticeInfo<T,Descriptor>::getSmallestBlock() const {
    return smallest;
}

template<typename T, template<typename T> class Descriptor>
Box3D MultiBlockLatticeInfo<T,Descriptor>::getLargestBlock() const {
    return largest;
}

}  // namespace plb

#endif  // SWIG_MULTI_BLOCK_LATTICE_INFO_3D_HH
